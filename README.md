...
The immediate challenge is the lack of data. Not only are comebacks quite rare in football, good quality match data is kept behind paywalls. The data used comes from the kind folks over at [Football Data](https://football-data.co.uk/). The data used is mostly betting-focused, so there are a lot of useful features that may be missing and has affected the model. Still, it's the best I can do for my budget (that is, nothing).
...
Football seasons bring radical changes. At the moment, the model is very biased towards the Big 6 clubs in the PL due to the fact that they've always been really good teams. This means that the model does not reflect (or have any idea about) the current performance of the teams in the current season.
